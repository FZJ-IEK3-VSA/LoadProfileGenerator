...
namespace CalcPostProcessor.LoadTypeHouseholdSteps {
    public class IndividualHouseholdNoFlexSumProfileProcessor : HouseholdLoadTypeStepBase {
        [JetBrains.Annotations.NotNull] private readonly IFileFactoryAndTracker _fft;

        public IndividualHouseholdNoFlexSumProfileProcessor([JetBrains.Annotations.NotNull] CalcDataRepository repository,
            ...
            repository,
            AutomationUtili.GetOptionList(CalcOption.HouseholdSumProfilesCsvNoFlex),
            calculationProfiler,
            "Individual Household No Flexibility Devices Sum Profiles") =>
            _fft = fft;
        ...
        protected override void PerformActualStep(IStepParameters parameters)
        {
            ...
            var sumfile = _fft.MakeFile<StreamWriter>("SumProfiles.NoFlex" + hhname + dstLoadType.Name + ".csv",
                "Summed up energy profile for all devices for " + dstLoadType.Name + " for " + hhname,
                true,
                ResultFileID.SumProfileForHouseholdsNoFlex,
                p.Key.HHKey,
                TargetDirectory.Results,
                Repository.CalcParameters.InternalStepsize, CalcOption.HouseholdSumProfilesCsvNoFlex,
