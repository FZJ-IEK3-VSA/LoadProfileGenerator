...
namespace Common.SQLResultLogging {
    ...
    public class SqlResultLoggingService {
        ...
        /*
        public void NpocoSaveToDatabase<T>(List<T> items) where T : ITypeDescriber
        {
            using (SQLiteConnection dbcon = new SQLiteConnection(GetFilenameForHouseholdKey()))
            {
                dbcon.CreateTable<T>(CreateFlags.AllImplicit);
            }
            Database db = GetDatabaseConnection();
            Stopwatch sw = new Stopwatch();
            sw.Start();

            SaveTableDescription(items, db);
            db.BeginTransaction();
        ...
        /*
        private static void SaveTableDescription<T>([JetBrains.Annotations.NotNull] [ItemNotNull]
                                                    List<T> items, [JetBrains.Annotations.NotNull] NPoco.Database db)
            where T : ITypeDescriber
        {
            TableDescription td = new TableDescription();
            td.TableName = typeof(T).Name;
            if (items.Count == 0) {
                td.Description = "not used";
            }
            else {
                td.Description = items[0].GetTypeDescription();
            }

            db.Save(td);
        }*/
        /*
        private void SaveToDatabase<T>([JetBrains.Annotations.NotNull][ItemNotNull] List<T> items, [JetBrains.Annotations.NotNull] string filename) where T : ITypeDescriber
        {
            using (SQLiteConnection dbcon = new SQLiteConnection(filename)) {
                dbcon.CreateTable<T>(CreateFlags.AllImplicit);
            }

            NPoco.Database db =
                new NPoco.Database("Data Source=" + filename, DatabaseType.SQLite, SQLiteFactory.Instance);
            Stopwatch sw = new Stopwatch();
            sw.Start();

            SaveTableDescription(items, db);
            using (System.Data.SQLite.SQLiteConnection conn =
                new System.Data.SQLite.SQLiteConnection(@"Data Source=" + filename +
                                                        ";Version=3;Synchronous=OFF;Journal Mode=WAL;")) {
                conn.Open();
                using (var transaction = conn.BeginTransaction()) {
                    using (var cmd = new System.Data.SQLite.SQLiteCommand(conn)) {
                        var properties = typeof(T).GetProperties()
                            .Where(x => x.Name != "ID" && x.Name != "HouseholdKey").ToList();
                        ...
                        Dictionary<string, SQLiteParameter> pars = new Dictionary<string, SQLiteParameter>();
                        foreach (var propertyInfo in properties) {
                            SQLiteParameter par = cmd.Parameters.AddWithValue("@" + propertyInfo.Name,
                                propertyInfo.GetValue(items[0]));
                            pars.Add(propertyInfo.Name, par);
                        }

                        foreach (var item in items) {
                            foreach (var propertyInfo in properties) {
                                pars[propertyInfo.Name].Value = propertyInfo.GetValue(item);
                            }
