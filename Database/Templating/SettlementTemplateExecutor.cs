...
namespace Database.Templating {
    public class SettlementTemplateExecutor {
            ...
            foreach (var housePreviewEntry in _previewHouseEntries) {
                ...
                foreach (var house in sim.Houses.Items) {
                    ...
                    foreach (var household in housePreviewEntry.Households) {
                        if (!calcObjects.Contains(household.Household)) {
                ...
                if (newhouse == null) {
                    ...
                    foreach (var household in housePreviewEntry.Households) {
                        newhouse.AddHousehold(household.Household, household.ChargingStationSet, household.TravelRouteSet,
            ...
            [NotNull] SettlementTemplate template)
        {
            ...
            if (template.ChargingStationSets.Count == 0)
            {
                throw new DataIntegrityException("Can't make a settlement with 0 charging stations.");
            }
            if (template.TransportationDeviceSets.Count == 0)
            {
                throw new DataIntegrityException("Can't make a settlement with 0 transportation device sets.");
            }
            if (template.TravelRouteSets.Count == 0)
            {
                throw new DataIntegrityException("Can't make a settlement with 0 travel route sets.");
            }
            ...
            foreach (var stHouseSize in template.HouseSizes) {
                for (var i = 0; i < stHouseSize.HouseCount; i++) {
                    ...
                    var housesize = stHouseSize.MinimumHouseSize + rnd.Next(span);
                    ...
                    for (var j = 0; j < housesize && resultinghouseholds.Count > 0; j++) {
                        var householdIndex = rnd.Next(resultinghouseholds.Count);
                        var hhe = MakeHouseEntry(resultinghouseholds[householdIndex], template, rnd);
                        he.Households.Add(hhe);
            ...
            foreach (var houseEntry in filteredEntries) {
                ...
                var idx = rnd.Next(validHouseTypes.Count);
        ...
        private static HouseholdEntry MakeHouseEntry(ModularHousehold household, [NotNull] SettlementTemplate template, [NotNull] Random rnd)
        {
            ...
            var travelRouteSet = template.TravelRouteSets[rnd.Next(template.TravelRouteSets.Count)];
            return new HouseholdEntry(household,transportationDeviceSet.TransportationDeviceSet,
                css,travelRouteSet.TravelRouteSet);
        }
            ...
            [NotNull] SettlementTemplate template)
        {
            ...
            if (fittingtemplates.Count == 0) {
                throw new DataIntegrityException("For the household with size >=" + distribution.MinimumNumber +
                                                 ...
                                                 "no household template was assigned in the settlement template " + template.PrettyName + "." );
            }
            ...
            [CanBeNull] HouseType ht)
        {
            ...
            foreach (var calcObject in households) {
                var name = calcObject.Household.Name;
            ...
            foreach (var distribution in template.HouseholdDistributions) {
                ...
                for (var i = 0; i < distribution.RoundedHHCount; i++) {
                    ...
                    if (matchingObjects.Count > 0) {
                        ...
                        var templateidx = rnd.Next(fittingtemplates.Count);
        ...
        public class HouseholdEntry {
            public HouseholdEntry(ModularHousehold household, TransportationDeviceSet transportationDeviceSet, ChargingStationSet chargingStationSet, TravelRouteSet travelRouteSet)
            {
                Household = household;
            ...
            public ModularHousehold Household { get;  }
        ...
        public sealed class HouseEntry : IComparable, IEquatable<HouseEntry> {
                ...
                EnergyIntensityType energyIntensityType)
            {
                ...
                Number = number;
                EnergyIntensityType = energyIntensityType;
                Households = new List<HouseholdEntry>();
            }
            ...
            public string HouseholdList {
                get {
                    var builder = new StringBuilder();
                    foreach (var calcObject in Households) {
                        builder.Append(calcObject.Household.Name).Append(", ");
                    }
