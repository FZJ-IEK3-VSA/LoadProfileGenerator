...
namespace CalculationController.Tests.CalcFactories
{
    public class CalcTransportationFactoryTests : UnitTestBaseClass
    {
        ...
        public void CheckRouteCompletenessTest()
        {
            ...
            List<Site> sites = new List<Site>();
            //test with empty
            CalcTransportationDtoFactory.CheckIfSitesAreFullyConnected(trs,sites);
            ...
            sites.Add(sitea);
            sites.Add(siteb);
            trs.AddRoute(new TravelRoute(1, "", "Route1", "desc", sitea, siteb, Guid.NewGuid().ToStrGuid(), null), 0, 80, Common.Enums.PermittedGender.All, null, null, 1.0, null, false);
            CalcTransportationDtoFactory.CheckIfSitesAreFullyConnected(trs, sites);

            //one route missing
            Site sitec = new Site("sitec", 2, "", "", true, Guid.NewGuid().ToStrGuid());
            sites.Add(sitec);
            trs.AddRoute(new TravelRoute(1, "", "Route2", "desc", siteb, sitec, Guid.NewGuid().ToStrGuid(), null), 0, 80, Common.Enums.PermittedGender.All, null, null, 1.0, null, false);
            Action  crashfunction1 = () => CalcTransportationDtoFactory.CheckIfSitesAreFullyConnected(trs, sites);
            crashfunction1.Should().Throw<DataIntegrityException>();
            //add missing route
            trs.AddRoute(new TravelRoute(1, "", "Route3", "desc", sitea, sitec, Guid.NewGuid().ToStrGuid(), null), 0, 80, Common.Enums.PermittedGender.All, null, null, 1.0, null, false);
            CalcTransportationDtoFactory.CheckIfSitesAreFullyConnected(trs, sites);
        }
